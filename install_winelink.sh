	...
	"ARM32" | "ARM64") ############### armhf & aarch64 OS Section ###############
		...
		"RPi4")
			...
			"raspbian" | "debian") # Pi4 with Raspberry Pi OS
				...
				"ARM32")
					run_greeting "${SBC_SERIES} ${ARCH} " "90" "2.0" "${ARG}" #Vars: "Hardware", "OS Bits", "Minutes", "GB", "bap" (check if user passed "bap" to script)
					run_piappswine ${ARCH}
					;; #/"ARM32")
				"ARM64")
		...
		"RPi3+" | "RPi3")
			...
			"raspbian" | "debian") # Pi3 with Raspberry Pi OS (64-bit)...
				case $ARCH in
				"ARM32")
					run_greeting "${SBC_SERIES} ${ARCH}" "160" "4.0" "${ARG}"
...
function run_buildbox64() { # This method is not currently used
	...
	cd box64
	rm -rf box64-builder
	mkdir box64-builder && cd box64-builder/
...
function run_buildbox86() { # Compile box64 & box86 on-device (takes a long time, builds are fresh and links less breakable)
	...
	cd box86
	rm -rf box86-builder
	mkdir box86-builder && cd box86-builder/
...
function run_Sideload_i386wine() {
	...
	rm -rf ~/wine-old 2>/dev/null
	mv ~/wine ~/wine-old 2>/dev/null
	rm -rf ~/.wine-old 2>/dev/null
	mv ~/.wine ~/.wine-old 2>/dev/null
...
function run_custompi3kernel() { # Needed to run wine on Pi3's running RPiOS (32-bit)
	...
	if [ -z "$vmsplit_output" ]; then
		...
		if [ "$kernelswapmethod" == 1 ]; then #install precompiled 3g/1g kernel
			...
			rm -rf ~/linux.bak
			[ -e ~/linux ] && (
				echo "$HOME/linux already exists, moving it to $HOME/linux.bak"
				mv -f ~/linux ~/linux.bak
			)
...
function run_installrms() { # Download/extract/install RMS Express
	...
	sleep 3
	sudo rm -rf ~/.local/share/applications/wine/Programs/RMS\ Express/ # Remove wine's auto-generated program icon from the start menu
...
function run_detect_os() {
	...
	if [ -e /etc/os-release ]; then
		OS_INFOFILE='/etc/os-release' #&& echo "Found an OS info file located at ${OS_INFOFILE}"
	elif [ -e /usr/lib/os-release ]; then
		OS_INFOFILE='/usr/lib/os-release' #&& echo "Found an OS info file located at ${OS_INFOFILE}"
	elif [ -e /etc/*elease ]; then
		OS_INFOFILE='/etc/*elease' #&& echo "Found an OS info file located at ${OS_INFOFILE}"
	...
	# Add chroot Android? (uname -o  can be used to find "Android")
		...
		OS_INFOFILE='' && echo "No Linux OS info files could be found!" >&2 && run_giveup
	fi
...
run_main "$@"
exit # Run the "run_main" function after all other functions have been defined in bash.  This allows us to keep our main code at the top of the script.
